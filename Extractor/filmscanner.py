...
frame1 = [
    ...
    [sg.Checkbox('Take Pictures', default=False,
                 key="takePictures")],
...
while True:
    ...
    if (video.frame is not None and video.cropped is not None and video.gray is not None and video.edges is not None and video.eDilate is not None and video.eErode is not None and video.cdst is not None and video.cropCopy is not None):
        if (viewNum == 0):
            imgbytes = cv2.imencode(".png", cv2.resize(
                video.cropped, (960, 640)))[1].tobytes()
        if (viewNum == 1):
            imgbytes = cv2.imencode(".png", cv2.resize(
